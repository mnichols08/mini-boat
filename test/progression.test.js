...
test('all three levels transition, persist once, and restart cleanly', () => {
  let now = 1000;
  const runs = [];
  const room = new Room({ now: () => now, runStore: { saveRun: run => runs.push(run) } });
  ...
  room.handleMessage(b, { type: 'restart' });
  assert.equal(room.status, 'countdown');
...
test('skipping the first checkpoint does not count a later one', () => {
  const sim = new BoatSimulation(); sim.loadLevel(1);
...
test('seat spoofing on row cannot operate the other oar', () => {
  const room = new Room(); const a = new Socket(), b = new Socket();
  room.addPlayer(a, 'A'); room.addPlayer(b, 'B'); room.status = 'playing';
...
test('replacement takes the vacant seat and starts a fresh run', () => {
  const room = new Room(); const a = new Socket(), b = new Socket();
  room.addPlayer(a, 'A'); room.addPlayer(b, 'B'); room.status = 'playing';
  room.removePlayer(a);
  ...
  room.addPlayer(new Socket(), 'C');
  assert.deepEqual(room.players.map(p => p.seat), ['right', 'left']);
  assert.equal(room.status, 'countdown');
});
