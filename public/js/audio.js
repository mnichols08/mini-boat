...
export class BoatAudio {
  ...
  async unlock() {
    ...
    try {
      if (!this.context) {
        ...
        this.noise = this.context.createBuffer(1, this.context.sampleRate * 0.3, this.context.sampleRate);
  ...
  toggleMute() {
    ...
    if (this.master) this.master.gain.setTargetAtTime(this.muted ? 0 : VIEW.audioVolume,
      this.context.currentTime, 0.02);
  }

  voice({ frequency = 400, endFrequency = frequency, duration = 0.16, delay = 0, noise = false, pan = 0, volume = 0.5 }) {
  ...
  play(kind, { side, strength = 0.5 } = {}) {
    if (this.muted || this.unavailable || this.context?.state !== "running") return;
    try {
      if (kind === "row") {
        this.voice({ noise: true, frequency: 1200, duration: 0.18, pan: side === "left" ? -0.45 : 0.45 });
      } else if (kind === "sync") {
        this.voice({ frequency: 440, duration: 0.22, volume: 0.28 });
        this.voice({ frequency: 660, delay: 0.04, duration: 0.24, volume: 0.22 });
      } else if (kind === "rock") {
        this.voice({ frequency: 135, endFrequency: 65, duration: 0.18, volume: 0.4 + strength * 0.3 });
      } else if (kind === "bank") {
        this.voice({ noise: true, frequency: 380, duration: 0.22, volume: 0.5 });
      } else {
        const notes = kind === "complete" ? [392, 494, 587] : [523, 659];
        notes.forEach((frequency, index) => this.voice({ frequency, delay: index * 0.11, duration: 0.28, volume: 0.3 }));
      }
    } catch (_error) {
